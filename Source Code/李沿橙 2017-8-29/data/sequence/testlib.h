...
int lemonPerfectScore;
FILE *lemonActualScore;
bool lemonMode;
...
NORETURN void InStream::quit(TResult result, const char* msg)
{
	...
	if (lemonMode)
	{
		if (result == _ok) fprintf(lemonActualScore, "%d", lemonPerfectScore);
		exit(0);
	}
...
NORETURN void __testlib_quitp(double points, const char* message)
{
	...
	{
		quitMessage = message;
		fprintf(lemonActualScore, "%f", points);
	}
	else
	{
...
NORETURN void __testlib_quitp(int points, const char* message)
{
	...
	{
		quitMessage = message;
		fprintf(lemonActualScore, "%d", points);
	}
	else
	{
		if (NULL == message || 0 == strlen(message))
...
void registerLemonChecker(int argc, char* argv[])
{
	...
	resultName = argv[6];
	appesMode = false;
	lemonPerfectScore = atoi(argv[4]);
	lemonActualScore = fopen(argv[5], "w");
}
